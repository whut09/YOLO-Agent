...
Status is measured in four independent layers. Every number is read from a machine-readable artifact or config, not hand-maintained. The readiness and reproduction blocks below are generated by `python -m yolo_agent.tools.docs_status`; hand edits inside the markers are overwritten, and the tool's `--check` mode fails when the committed text drifts. The combined report lives at [docs/generated/current-status.md](docs/generated/current-status.md) (Chinese: [current-status.zh-CN.md](docs/generated/current-status.zh-CN.md)).
