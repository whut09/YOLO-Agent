...
def test_pareto_uses_only_current_verified_local_training_evidence() -> None:
    report = build_paper_pareto_report([
        ...
        PaperParetoCandidate(candidate_id="slicing", recipe_id="r5", verified_local=True, slicing_metrics={"sliced_map50_95": 0.70, "sliced_ap_small": 0.31, "sliced_latency_ms": 30.0, "sliced_throughput": 33.3}),
    ])
    ...
    assert [item.candidate_id for item in report.inference_included] == ["slicing"]
    assert report.inference_included[0].inference_policy_changed is True
