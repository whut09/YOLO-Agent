...
不加 `--execute` 时就是 dry-run：

```powershell
yolo-agent optimize coco --model yolo26n.pt --data E:\datatset\coco.yaml --run-id coco-yolo26n --profile debug
```
...
```powershell
yolo-agent optimize coco --model yolo26n.pt --data E:\datatset\coco.yaml --run-id coco-yolo26n --profile debug --execute --no-auto-advance
```
